...
The desktop UI features switchable Overview/Detail modes for blocs, policies, and the sidebar, plus a guided tutorial with spotlight highlighting. Fully responsive for mobile with a tab-based layout.
...
- **Build Tool:** Vite
- **Language:** TypeScript
- **Testing:** Vitest with seedable PRNG (85 unit tests across 4 suites, including 3,000-seed fuzz across 3 difficulty levels) + cross-browser smoke tests (Chromium, Firefox, WebKit)
...
```bash
npm test              # 85 unit tests (engine logic, fuzz, balance)
